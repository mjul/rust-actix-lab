...
async fn main() {
    ...
    let rt_minutes_at = |rt: time::Duration, day, hh, mm| -> rust_actix_labx::ResponseTime {
        let t = time::PrimitiveDateTime::new(time::Date::from_calendar_date(2022, time::Month::July, day).unwrap(), time::Time::from_hms(hh, mm, 0).unwrap());
        rust_actix_labx::ResponseTime::new(rt, t)
    };

    for i in 0..100 {
        let req = rust_actix_labx::ObserveResponseTimeRequest::new(rt_minutes_at(time::Duration::seconds(180+i), 1, 12, (i%60) as u8));
        let _ = addr.send(req).await;
    }

    let resp = addr.send(rust_actix_labx::ResponseTimeP95Request::new()).await;
    let p95 = resp.unwrap();
